					...
					if($url == NULL) {	
						...
						if($cc == NULL) {
							...
							if($network == NULL) {
								...
								$addOffer = mysqli_query($conn,"INSERT INTO `offers` (`id`,`offerId`,`name`, `url`, `payout`,`ratio`, `imageUrl`,`des`,`country`,`scam`,`view`, `network`, `status`,`OS`) VALUES('','$offerId','$name','$url','$payout','$ratio','$image_url', '$des','$cc','$scam','$view', '$network', 'ON','link')") or die(mysqli_error()); 				
...
<div style="height:40px;margin:5px;">
	<div style="float:left;">
		<form action="./index.php?offers=link" method="GET">
			...
			<span class="reset"><a href="index.php?offers=link">Reset Filter</a></span>
		</form>
	</div>
...
<?php
	...
	if(isset($_POST['delOffers']) && isset($_POST['id'])){
        ...
        header("Location: index.php?offers=link");
	}
...
?>
			...
			<form action="./index.php?offers=link" method="POST">
			<table style="max-width:808px;" id="list_members" cellpadding="0" cellspacing="0" border="0" class="table table-bordered table-striped table-condensed">
				...
				<?php 
						if(!isset($_GET['filter'])) {
							$queryoffers = mysqli_query($conn,"SELECT * FROM offers WHERE `OS`='link'") or die(mysqli_error());
						} else {
							if($_GET['country']!=NULL && $_GET['network']==NULL) {
								$ccode = addslashes($_GET['country']);
								$queryoffers = mysqli_query($conn,"SELECT * FROM offers WHERE `OS`='link' and country='".$ccode."'") or die(mysqli_error());
							} else {
								if($_GET['country']==NULL && $_GET['network']!=NULL) {
									$nwk = addslashes($_GET['network']);
									$queryoffers = mysqli_query($conn,"SELECT * FROM offers WHERE `OS`='link' and network='".$nwk."'") or die(mysqli_error());
								} else {
									$ccode = addslashes($_GET['country']);
									$nwk = addslashes($_GET['network']);
									$queryoffers = mysqli_query($conn,"SELECT * FROM `offers` WHERE (`OS`='link' and `country`='$ccode' AND `network`='$nwk')") or die(mysqli_error());
								}
							}
						}
