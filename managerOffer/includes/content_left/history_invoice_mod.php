<?php
	...
	if(isset($_GET['offerId'])&&$_GET['offerId']!="")
	{
		...
		$query_by_name=" and invoice.offerName='".$row_offer_name['name']."'";
	}
	else
	if(isset($_GET['network'])&&$_GET['network']!="")
	{
		...
		$query_by_name=" and invoice.network='".addslashes($_GET['network'])."'";
...
<div class="alert alert-success" style="padding: 14px 5px 14px 10px !important;">
		...
		<div class="input_form_filter">
			USERNAME <input type="text" style="width:100px !important" name="userName" class="input-xlarge " value="<?php echo $userName;?>">
		</div>
		<div class="input_form_filter">
...
<table id="list_members" cellpadding="0" cellspacing="0" border="0" id="table" class="table table-bordered table-striped table-condensed">
	  ...
	  ?>
			  <tr><th>UserName</th><?php if($group_by_name==",network"){echo "<th>Network</th>";}if($group_by_name==",offerName"){echo "<th>Offer Name</th>";}?><th>Point</th><th>Money</th><th>From</th><th>To</th><th>Date create</th><th>Status</th></tr>
	<?php 		
	$sumUsers=mysqli_query($conn,"Select invoice.points,invoice.userName,invoice.offerName,invoice.network,`invoice`.`from`,`invoice`.`to`,invoice.date_create,invoice.status from invoice where DATE(invoice.date_create)>='$from' and DATE(invoice.date_create)<='$to' $query_by_name order by invoice.userName,invoice.points desc");
	$sumPoint=0;
