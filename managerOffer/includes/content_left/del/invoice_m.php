...
<div class="col-md-12">
    <div class="alert alert-success">
		<form action="index.php?invoice=month" method="get" style="display:inline">
			...
			<a href="./export_invoice.php?month=<?php echo $m_30day;?>" target="_blank" class="btn btn-warning" />EXPORT</a>
	</div>
</div>
...
<table id="list_affiliate" cellpadding="0" cellspacing="0" border="0" id="table" class="table table-bordered table-striped table-condensed">
	  ...
	  ?>
		...
		if(mysqli_num_rows($query_check_invoice))
		{
			...
			$query_invoice=mysqli_query($conn,"Select invoice.status,invoice.userName,invoice.leads,invoice.clicks,invoice.amount_w,invoice.cvr,invoice.rpc,invoice.rpa,invoice.network from invoice inner join networks ON networks.name=invoice.network where month='$m_30day' and networks.payment='month'");
			while($invoice = mysqli_fetch_array($query_invoice)) 
			{
				if($invoice['status']=="unpaid")
				{
					$create_invoice="<span class='paid_".$invoice['userName']."'><a class='btn btn-danger button_tb manager mousepointer ' onclick=\"paid_user_m('".$invoice['userName']."','$m_30day')\">UNPAID</a></span>";
				}
				else
				{
					$create_invoice="<span class='paid_".$invoice['userName']."'><a  class='mousepointer btn btn-info button_tb manager' onclick=\"unpaid_user_m('".$invoice['userName']."','$m_30day')\">PAID</a></span>";
				}
