...
if ((!isset($_GET['token'])) || ($_GET['token'] != $session->data['token'])) 	{
		...
		foreach ($query->rows as $product) {
			...
			foreach ($tags as $tag)
				...
				if (strlen($tag) > 2) 
					{
					if ((strpos($product['tag'], ucfirst($tag)) === false) && (strpos($newtags, ucfirst($tag)) === false))
						{
							$newtags .= ' '.ucfirst($tag).',';
							$new++; $newp++;
						}			
			...
			if ($product['tag']) {
				...
				$db->query("update " . DB_PREFIX . "product_description set tag = '$newtags' where product_id = '". $product['product_id'] ."';");
				}
				else {
