<?php
	...
	if(isset($_GET['week']))
	{
		...
		$Y_week_7day = $date_today->format("Y-").$week_7day;
	}
	else
	{
...
<div class="col-md-12">
    <div class="alert alert-success">
		<form action="index.php?invoice=week" method="get" style="display:inline">
			...
			<a href="./export_invoice.php?week=<?php echo $week_7day;?>" target="_blank" class="btn btn-warning" />EXPORT</a>
	</div>
</div>
...
<table id="list_affiliate" cellpadding="0" cellspacing="0" border="0" id="table" class="table table-bordered table-striped table-condensed">
	  ...
	  ?>
		...
		if(mysqli_num_rows($query_check_invoice))
		{
			...
			$query_invoice=mysqli_query($conn,"Select invoice.status,invoice.userName,invoice.leads,invoice.clicks,invoice.amount_w,invoice.cvr,invoice.rpc,invoice.rpa,invoice.network from invoice inner join networks ON networks.name=invoice.network where week='$week_7day' and networks.payment='week' order by invoice.network,invoice.userName") or die (mysqli_error());
			while($invoice = mysqli_fetch_array($query_invoice)) 
			{
				if($invoice['status']=="unpaid")
				{
					$create_invoice="<span class='paid_".$invoice['userName']."'><a class='btn btn-danger button_tb manager mousepointer ' onclick=\"paid_user_w('".$invoice['userName']."','$week_7day')\">UNPAID</a></span>";
				}
				else
				{
					$create_invoice="<span class='paid_".$invoice['userName']."'><a  class='mousepointer btn btn-info button_tb manager mousepointer ' onclick=\"unpaid_user_w('".$invoice['userName']."','$week_7day')\">PAID</a></span>";
				}
		...
		else
		{
			...
			if(mysqli_num_rows($invoices_query))
			{
				...
				$query_invoice=mysqli_query($conn,"Select invoice.status,invoice.userName,invoice.leads,invoice.clicks,invoice.amount_w,invoice.cvr,invoice.rpc,invoice.rpa,invoice.network from invoice whereinner join networks ON networks.name=invoice.network week='$week_7day' and networks.payment='month'");
				while($invoice = mysqli_fetch_array($query_invoice)) 
				{
					if($invoice['status']=="unpaid")
					{
						$create_invoice="<span class='paid_".$invoice['userName']."'><a class='mousepointer btn btn-danger button_tb manager mousepointer ' onclick=\"paid_user_w('".$invoice['userName']."','$week_7day')\">UNPAID</a></span>";
					}
					else
					{
						$create_invoice="<span class='paid_".$invoice['userName']."'><a  class='mousepointer btn btn-info button_tb manager mousepointer ' onclick=\"unpaid_user_w('".$invoice['userName']."','$week_7day')\">PAID</a></span>";
