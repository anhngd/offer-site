<?php
	if(isset($_POST['sent_chat'])&&(isset($_SESSION['isAdmin'])||isset($_SESSION['isMod'])))
	{
		...
		mysql_query("Insert into notify(`userName`,`groupName`,`manager`,`content`,`date`) value('$userName','$groupName','$manager','$content','$date')") or die(mysql_error());
	}
?>
<div class="box blue span3" ontablet="span6" ondesktop="span3">
	...
	if(isset($_SESSION['isAdmin']))
	{
		$notify_query=mysql_query("Select * from notify order by id DESC limit 0,5") or die (mysql_error());
	}
	else
	if(isset($_SESSION['isMod']))
	{
		$notify_query=mysql_query("Select * from notify where groupName='$groupName' or manager=1 order by id DESC limit 0,5") or die (mysql_error());
	}
	else
	{
		$notify_query=mysql_query("Select * from notify where groupName='$groupName' or manager=1 order by id DESC limit 0,5") or die (mysql_error());
	...
	<?php
	while($notify=mysql_fetch_array($notify_query))
	{
...
?>
	...
	<div class="box-content">
		<div id="box_notify_leads">
			<?php
			include("./includes/content_right/notify_leads.php");
			?>
		</div>
	</div>
</div>
