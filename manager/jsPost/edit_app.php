...
if(isset($_POST['name_app']))
{
	...
	$query_add_app=mysqli_query($conn,"Update app_info set `name`='$name_app',`version`='$version',`size`='$size',`link_offer`='$link_offer',`producer`='$producer',`view`='$view',`link_img`='$file_name',`OS`='$os',`date_update`='$date_update',`content`='$content_app',`hot`='$hot',status='$status' where id='$id'");
	if($query_add_app)
	{
		echo "oke";
	}
