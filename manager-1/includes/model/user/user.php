...
class ModelUserUser extends Model {
	...
	public function getUserGroup() {		
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "user` WHERE user_id = '" . $this->user->getId() . "'");			
		return $query->row['user_group_id'];
	}
	public function getUsers($data = array()) {	
		$gquery = $this->db->query("SELECT * FROM `" . DB_PREFIX . "user` WHERE user_id = '" . (int)$data['uid'] . "'");	
		
		$group = $gquery->row['user_group_id'];
		if($group==1){
		$sql = "SELECT * FROM `" . DB_PREFIX . "user`";		
		}else{//demo
		$sql = "SELECT * FROM `" . DB_PREFIX . "user` WHERE user_group_id='" . $group . "' AND user_id='" . (int)$data['uid'] . "'";
		}//demo		
