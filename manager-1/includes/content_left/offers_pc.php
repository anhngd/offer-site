					...
					if($url == NULL) {	
						...
						if($cc == NULL) {
							...
							if($network == NULL) {
								...
								$addOffer = mysqli_query($conn,"INSERT INTO `offers` (`id`,`offerId`,`name`, `url`, `payout`,`ratio`, `imageUrl`,`des`, `network`, `status`,`OS`,`end_tracking`,`show_index`,`min_os_version`,`app_size`,`view`,`update_date`,`hot`,`category`,`producer`) VALUES('','$offerId','$name','$url','$payout','$ratio','$image_url', '$des', '$network', 'ON','pc','$end_tracking','$show_index','$min_os_version','$app_size','$view','$update_date','$hot','$category','$producer')") or die(mysqli_error());
...
<div style="height:40px;margin:5px;">
	<div style="float:left;">
		<form action="./index.php?offers=pc" method="GET">
			<input type="hidden" value="pc" name="offers"/>
			<select name="network">
			...
			<span class="reset"><a href="index.php?offers=pc">Reset Filter</a></span>
		</form>
	</div>
...
<?php
	...
	if(isset($_POST['delOffers']) && isset($_POST['id'])){
        ...
        header("Location: index.php?offers=pc");
	}
...
?>
			...
			<form action="./index.php?offers=pc" method="POST">
			<table  id="list_members" cellpadding="0" cellspacing="0" border="0" class="table table-bordered table-striped table-condensed">
				...
				<?php 
						if(!isset($_GET['filter'])) {
							$queryoffers = mysqli_query($conn,"SELECT * FROM offers WHERE `OS`='pc'") or die(mysqli_error());
						} else {
							$_GET['country']="";
							if($_GET['country']!=NULL && $_GET['network']==NULL) {
								//$ccode = addslashes($_GET['country']);
								$queryoffers = mysqli_query($conn,"SELECT * FROM offers WHERE `OS`='pc'") or die(mysqli_error());
								//$queryoffers = mysqli_query($conn,"SELECT * FROM offers WHERE `OS`='pc' and country='".$ccode."'") or die(mysqli_error());
							} else {
								if($_GET['country']==NULL && $_GET['network']!=NULL) {
									$nwk = addslashes($_GET['network']);
									$queryoffers = mysqli_query($conn,"SELECT * FROM offers WHERE `OS`='pc' and network='".$nwk."'") or die(mysqli_error());
								} else {
									//$ccode = addslashes($_GET['country']);
									$nwk = addslashes($_GET['network']);
									//$queryoffers = mysqli_query($conn,"SELECT * FROM `offers` WHERE (`OS`='pc' and `country`='$ccode' AND `network`='$nwk')") or die(mysqli_error());
									$queryoffers = mysqli_query($conn,"SELECT * FROM `offers` WHERE (`OS`='pc' and `network`='$nwk')") or die(mysqli_error());
								}
							}
						}
