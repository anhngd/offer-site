<?php
	...
	if($username!="")
	{
		...
		$query_groupname="AND groupName='".$groupName."'";

	$sumClicks=mysqli_query($conn,"Select id from clicks where DATE(date)>='$from' and DATE(date)<='$to' $query_username $query_groupname $query_nwk");
	$clicks_query=mysqli_query($conn,"Select * from clicks where DATE(date)>='$from' and DATE(date)<='$to' $query_username $query_groupname $query_nwk ORDER BY id DESC limit $limit,$numSplitPage") or die (mysqli_error());
?>
