					...
					if($url == NULL) {	
						...
						if($cc == NULL) {
							...
							if($network == NULL) {
								...
								$addOffer = mysqli_query($conn,"INSERT INTO `offers` (`id`,`offerId`,`name`, `url`, `payout`,`ratio`, `imageUrl`,`des`,`country`,`scam`,`view`, `network`, `status`,`OS`) VALUES('','$offerId','$name','$url','$payout','$ratio','$image_url', '$des','$cc','$scam','$view', '$network', 'ON','survey')") or die(mysqli_error());
				...
				if($url == NULL) {	
					...
					if($cc == NULL) {
						...
						if($network == NULL) {
							...
							$update_offer = mysqli_query($conn,"UPDATE offers SET offerId ='".$offerId."', name ='".$name."', des ='".$des."', payout ='".$payout."', ratio ='".$ratio."', url ='".$url."', imageUrl ='".$image_url."', country ='".$cc."', network ='".$network."' , status ='".$status."',view ='".$view."', scam ='".$scam."' WHERE id ='".$id."'") or die(mysqli_error()); 				
...
<div style="height:40px;margin:5px;">
	<div style="float:left;">
		<form action="./index.php?offers=survey" method="GET">
			...
			<span class="reset"><a href="index.php?offers=survey">Reset Filter</a></span>
		</form>
	</div>
...
<?php
	...
	if(isset($_POST['delOffers']) && isset($_POST['id'])){
        ...
        header("Location: index.php?offers=survey");
	}
...
?>
			...
			<form action="./index.php?offers=survey" method="POST">
			<table style="max-width:808px;" id="list_members" cellpadding="0" cellspacing="0" border="0" class="table table-bordered table-striped table-condensed">
				...
				<?php 
						if(!isset($_GET['filter'])) {
							$queryoffers = mysqli_query($conn,"SELECT * FROM offers WHERE `OS`='survey'") or die(mysqli_error());
						} else {
							if($_GET['country']!=NULL && $_GET['network']==NULL) {
								$ccode = addslashes($_GET['country']);
								$queryoffers = mysqli_query($conn,"SELECT * FROM offers WHERE `OS`='survey' and country='".$ccode."'") or die(mysqli_error());
							} else {
								if($_GET['country']==NULL && $_GET['network']!=NULL) {
									$nwk = addslashes($_GET['network']);
									$queryoffers = mysqli_query($conn,"SELECT * FROM offers WHERE `OS`='survey' and network='".$nwk."'") or die(mysqli_error());
								} else {
									$ccode = addslashes($_GET['country']);
									$nwk = addslashes($_GET['network']);
									$queryoffers = mysqli_query($conn,"SELECT * FROM `offers` WHERE (`OS`='survey' and `country`='$ccode' AND `network`='$nwk')") or die(mysqli_error());
								}
							}
						}
